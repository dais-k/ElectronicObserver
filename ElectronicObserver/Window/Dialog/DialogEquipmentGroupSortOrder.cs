...
{
	public partial class DialogEquipmentGroupSortOrder : Form
	{
		...
		public DialogEquipmentGroupSortOrder(DataGridView target, EquipmentGroupData group)
		{
			InitializeComponent();
			...
			foreach (var sort in group.SortOrder.Where(s => names.Contains(s.Key)))
			{
				...
				string headerText = target.Columns[sort.Key].HeaderText;
				if (sort.Key == "EquipView_Icon")
				{
					headerText = "アイコン";
				}
				row.SetValues(headerText, sort.Value);
				...
				.Select(c => c.Name))
			{
				...
				string headerText = target.Columns[name].HeaderText;
				if (name == "EquipView_Icon")
				{
					headerText = "アイコン";
				}
				row.SetValues(headerText);
		...
		private void DialogEquipmentGroupSortOrder_Load(object sender, EventArgs e)
		{
			if (Owner != null)
				Icon = Owner.Icon;
		}
