...
{
	public partial class DialogEquipmentGroupColumnFilter : Form
	{

		public List<EquipmentGroupData.ViewColumnData> Result { get; private set; }
		public int ScrollLockColumnCount { get; private set; }

		public DialogEquipmentGroupColumnFilter(DataGridView target, EquipmentGroupData group)
		{
			InitializeComponent();
			...
			foreach (var c in group.ViewColumns.Values.OrderBy(c => c.DisplayIndex))
			{
				...
				row.CreateCells(ColumnView);
				// "EquipView_Icon" の場合は"アイコン"を渡す
				string headerText = target.Columns[c.Name].HeaderText;
				if (c.Name == "EquipView_Icon")
				{
					headerText = "アイコン";
				}
				row.SetValues(headerText, c.Visible, c.AutoSize, c.Width); row.Cells[ColumnView_Width.Index].ValueType = typeof(int);
				row.Tag = c.Name;
				rows.AddLast(row);
			}
		...
		private void DialogEquipmentGroupColumnFilter_Load(object sender, EventArgs e)
		{
			if (Owner != null)
				Icon = Owner.Icon;
		}
		...
		private void ButtonOK_Click(object sender, EventArgs e)
		{
			...
			Result = new List<EquipmentGroupData.ViewColumnData>(ColumnView.Rows.Count - 1);

			for (int i = 1; i < ColumnView.Rows.Count; i++)
			{

				var row = ColumnView.Rows[i];
				var r = new EquipmentGroupData.ViewColumnData((string)row.Tag)
				{
